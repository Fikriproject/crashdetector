...
#include <WiFiManager.h> // Library untuk captive portal WiFi
...
const char* server_url_post = "http://192.168.1.6/iot_accident_system/web/api/post_data.php";
const char* server_url_get  = "http://192.168.1.6/iot_accident_system/web/api/get_config.php";
...
void checkModuleHealth() {
  if (millis() - last_recovery_attempt > 5000) {
    ...
    // Sedot GPS setelah block SIM800L (bisa tertahan 200ms)
...
void setup() {
  ...
  // HAPUS TANDA // PADA BARIS DI BAWAH INI JIKA INGIN MERESET WIFI (MEMAKSA PORTAL MUNCUL)
  // wifiManager.resetSettings(); 

  Serial.println("Menghubungkan ke WiFi...");
  // Membuka Access Point "MotoSafe-Setup" jika belum ada WiFi tersimpan
  if (!wifiManager.autoConnect("MotoSafe-Setup")) {
    Serial.println("Gagal terhubung ke WiFi, restart...");
    delay(3000);
    ESP.restart();
  }
...
void reconnectMQTT() {
  if (!mqtt.connected()) {
    ...
    // Sedot GPS setelah mencoba konek MQTT (blocking)
...
void loop() {
  ...
  if (!ntp_synced && WiFi.status() == WL_CONNECTED && rtc_ok) {
    ...
    if (now > 1000000000) {
      struct tm timeinfo;
      // Gunakan localtime_r alih-alih gmtime_r agar sinkronisasi 
      // mengikuti zona waktu UTC+7 yang sudah diatur di configTime
      localtime_r(&now, &timeinfo);
      rtc.adjust(DateTime(timeinfo.tm_year + 1900, timeinfo.tm_mon + 1, timeinfo.tm_mday, timeinfo.tm_hour, timeinfo.tm_min, timeinfo.tm_sec));
      Serial.println("[NTP] Sinkronisasi Sukses! RTC diperbarui ke jam internet (Lokal).");
      ntp_synced = true;
    }
  }
  ...
  if (millis() - last_mqtt_publish > 500) {
    unsigned long current_ms = millis();
    String payload = "{";
    payload += "\"msg_id\":" + String(current_ms) + ",";
    ...
    if (mqtt.publish(topic.c_str(), payload.c_str())) {
      // Print diaktifkan agar mudah dicari
  ...
  if (millis() - last_mqtt_status_publish > 5000) {
    ...
    // Print disembunyikan agar log lebih rapi
  ...
  // --- BLOK MONITORING TERPADU (RTC, GPS, MPU, POST) ---
  static unsigned long last_monitor_print = 0;
  if (millis() - last_monitor_print >= 1000) {
    last_monitor_print = millis();
    
    // 1. Ambil Waktu dari RTC
    String rtc_time = "ERROR";
    if (rtc_ok) {
      DateTime now_rtc = rtc.now();
      char buf[25];
      snprintf(buf, sizeof(buf), "%04d-%02d-%02d %02d:%02d:%02d", now_rtc.year(), now_rtc.month(), now_rtc.day(), now_rtc.hour(), now_rtc.minute(), now_rtc.second());
      rtc_time = String(buf);
    }
    
    // 2. Ambil Waktu Real (NTP)
    String real_time = "BELUM SINKRON";
    time_t now_ntp = time(nullptr);
    if (now_ntp > 1000000000) {
      struct tm timeinfo;
      localtime_r(&now_ntp, &timeinfo);
      char buf_ntp[25];
      snprintf(buf_ntp, sizeof(buf_ntp), "%04d-%02d-%02d %02d:%02d:%02d", timeinfo.tm_year + 1900, timeinfo.tm_mon + 1, timeinfo.tm_mday, timeinfo.tm_hour, timeinfo.tm_min, timeinfo.tm_sec);
      real_time = String(buf_ntp);
    }
    
    Serial.println("===========================================================");
    Serial.print("[TIME] RTC: "); Serial.print(rtc_time);
    Serial.print(" | NTP: "); Serial.println(real_time);
    Serial.print("[GPS]  Lat: "); Serial.print(lat, 6);
    Serial.print(" | Lon: "); Serial.print(lng, 6);
    Serial.print(" | Alt: "); Serial.print(alt, 1);
    Serial.print(" | Spd: "); Serial.println(spd, 1);
    Serial.print("[MPU]  Pitch: "); Serial.print(pitch, 1);
    Serial.print(" | Roll: "); Serial.print(roll, 1);
    Serial.print(" | Yaw: "); Serial.print(yaw, 1);
    Serial.print(" | Accel: "); Serial.println(total_accel, 2);
    Serial.print("[POST] MPU:"); Serial.print(mpu_ok ? "OK" : "ERR");
    Serial.print(" | RTC:"); Serial.print(rtc_ok ? "OK" : "ERR");
    Serial.print(" | SD:"); Serial.print(sd_ok ? "OK" : "ERR");
    Serial.print(" | SIM:"); Serial.println(sim_ok ? "OK" : "ERR");
    Serial.println("===========================================================");
  }

  // --- PENYEDOT GPS TAMBAHAN ---
  // Sedot data GPS lagi setelah program tertahan oleh lamanya proses print di atas.
  while (Serial.available() > 0) {
    gps.encode(Serial.read()); 
  }
}
...
void handleAccident(float lat, float lng, float spd, float alt, float p, float r, float y, float a, float t) {
  ...
  Serial.println("\n[WAKTU] Kecelakaan Tervalidasi. Memulai pengiriman notifikasi...");
  ...
  Serial.print("[WAKTU] Waktu proses notifikasi web (HTTP): ");
  Serial.print(http_time);
  Serial.println(" ms");
  
  String time_str = getFormattedDateTime();
  if (sim_ok) {
    for(int i=0; i<contact_count; i++) {
      unsigned long sms_start = millis();
      sendSMS(emergency_contacts[i], lat, lng, time_str);
      unsigned long sms_time = millis() - sms_start;
      Serial.print("[WAKTU] Waktu proses SMS ke kontak ");
      Serial.print(i + 1);
      Serial.print(": ");
      Serial.print(sms_time);
      Serial.println(" ms");
    }
  } else {
  ...
  Serial.print("[WAKTU] Total waktu eksekusi seluruh notifikasi (Web + SMS): ");
...
void logToSD(float lat, float lng, float alt, float spd, float roll, float pitch, float yaw, float accel, float tmp, String status) {
  ...
  if (dataFile) {
    // Tambahkan header jika file baru dibuat (ukurannya 0)
    if (dataFile.size() == 0) {
      dataFile.println("Tanggal_Waktu;Latitude;Longitude;Altitude;speed;Roll;Pitch;Yaw;Total_Accel;Suhu;Status");
    }
    
    // Gunakan pemisah titik koma (;) agar langsung terbagi per kolom di Excel Indonesia
...
void sendHTTPPost(float lat, float lng, float spd, float alt, float p, float r, float y, float a, float t, String status) {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (asyncClient.connect(host.c_str(), port)) {
      // Sedot GPS yang tertumpuk selama proses koneksi HTTP berlangsung
      while (Serial.available() > 0) { gps.encode(Serial.read()); }
...
// Fungsi delay yang tetap memproses task background seperti MQTT, WDT, dan pembacaan GPS
void smartDelay(unsigned long ms) {
  unsigned long start = millis();
  do {
    // Terus sedot data GPS selama masa delay
    while (Serial.available() > 0) {
      gps.encode(Serial.read());
    }
    mqtt.loop();
