...
void setup() {
  ...
  Serial.begin(9600);
  Serial.println("========================================");
  Serial.println("  TEST CASE 08: Safe Mode & Recovery");
  Serial.println("========================================"); 
...
void loop() {
  // --- AWAL: SIMULASI PENGUJIAN WDT (BUTIR 18) ---
  if (millis() > 60000) {
    Serial.println();
    Serial.println(F("=================================================="));
    Serial.println(F("  MEMICU FREEZE (SISTEM HANG) UNTUK UJI WDT..."));
    Serial.println(F("  Alat akan sengaja dibuat berhenti merespons."));
    Serial.println(F("  Tunggu ~3 hingga 8 detik sampai terjadi wdt reset."));
    Serial.println(F("=================================================="));
    while(true) {
      // Infinite loop memblokir CPU tanpa memanggil ESP.wdtFeed()
    }
  }
  // --- AKHIR: SIMULASI PENGUJIAN WDT ---
