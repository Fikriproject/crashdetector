...
// === UJI LEAD TIME ===
unsigned long t0_jatuh = 0;
unsigned long t1_konfirmasi = 0;
...
void loop() {
  ...
  if (is_crashing) {
    if (!potential_accident) {
      ...
      // === UJI LEAD TIME ===
      t0_jatuh = millis();
      Serial.println("\n[UJI LEAD TIME] T0: Alat Jatuh / Benturan Terdeteksi");
    } else {
      if (millis() - accident_timer_start > 2000 && !accident_detected) {
        accident_detected = true;
        // === UJI LEAD TIME ===
        t1_konfirmasi = millis();
        Serial.print("[UJI LEAD TIME] T1: Konfirmasi Kecelakaan. Delay konfirmasi: ");
        Serial.print(t1_konfirmasi - t0_jatuh);
        Serial.println(" ms");
...
void sendSMS(String phone, float lat, float lng, String time_str) {
  Serial.print("[UJI LEAD TIME] Mulai proses kirim SMS ke: "); Serial.println(phone);
  ...
  // === UJI LEAD TIME ===
  unsigned long t_selesai = millis();
  Serial.print("[UJI LEAD TIME] Selesai proses kirim SMS ke: "); Serial.println(phone);
  Serial.print("[UJI LEAD TIME] -----> TOTAL WAKTU (Lead Time) dari T0 (Jatuh) ke SMS Terkirim: ");
  Serial.print(t_selesai - t0_jatuh);
  Serial.println(" ms\n");
}
